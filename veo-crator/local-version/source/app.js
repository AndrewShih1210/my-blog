...
function play(fromStart = false) {
  ...
  if (state.narration.audioUrl && els.narrationAudio.src) {
    els.narrationAudio.currentTime = Math.min(state.currentTime, els.narrationAudio.duration || state.currentTime);
    els.narrationAudio.volume = Number($("#previewVolume").value) / 100;
    els.narrationAudio.play().catch(() => showToast("請先在 AI 旁白面板播放一次音訊"));
  }
  drawSelection();
  const tick = now => {
...
function syncNarrationUI() {
  ...
  $("#narrationVoice").value = state.narration.voice || "zh-TW-HsiaoChenNeural";
  ...
  if (state.narration.audioUrl) {
    els.narrationAudio.src = state.narration.audioUrl;
    $("#narrationResult").hidden = false;
    $("#narrationStatus").textContent = "旁白已連結至專案";
  } else {
    els.narrationAudio.removeAttribute("src");
    $("#narrationResult").hidden = true;
  }
}
...
async function recordVideo(outputFormat = "webm") {
  ...
  try {
    ...
    if (state.narration.audioUrl) {
      recordingAudio = new Audio(state.narration.audioUrl);
      recordingAudio.crossOrigin = "anonymous";
      await new Promise((resolve, reject) => {
        recordingAudio.addEventListener("canplaythrough", resolve, { once: true });
        recordingAudio.addEventListener("error", reject, { once: true });
        recordingAudio.load();
      });
    ...
    if (outputFormat === "mp4") {
      els.recordStatus.textContent = "正在使用本機 ffmpeg 轉換 MP4…";
      ...
      if (!response.ok) {
        ...
        throw new Error(detail.error || "MP4 轉換失敗");
      }
      downloadBlob(await response.blob(), `${safeName(els.projectTitle.value)}.mp4`);
      els.recordStatus.textContent = "錄製完成，MP4 已下載";
      showToast("MP4 影片已下載");
    } else {
...
async function checkRuntimeStatus() {
  ...
  try {
    ...
    status.textContent = runtime.edgeTts && runtime.ffmpeg
      ? "Edge TTS 與 MP4 轉檔均已就緒"
      : `環境檢查：Edge TTS ${runtime.edgeTts ? "已就緒" : "缺少"}；ffmpeg ${runtime.ffmpeg ? "已就緒" : "缺少"}`;
    status.dataset.ready = String(runtime.edgeTts && runtime.ffmpeg);
  } catch {
    status.textContent = "請用 start-server.ps1 啟動，才能使用旁白與 MP4";
...
async function generateNarration() {
  ...
  try {
    const response = await fetch("/api/tts", {
      ...
      body: JSON.stringify({
        text,
        voice: $("#narrationVoice").value,
        rate: Number($("#narrationRate").value)
      })
    });
    const result = await response.json().catch(() => ({}));
    if (!response.ok) throw new Error(result.error || "旁白產生失敗");
    state.narration = {
      text,
      voice: result.voice,
      rate: result.rate,
    ...
    syncNarrationUI();
    $("#narrationStatus").textContent = "旁白已就緒，可加入預覽與影片";
    markChanged();
    showToast("Edge TTS 旁白已產生");
  } catch (error) {
    console.error(error);
    showToast(error.message || "旁白產生失敗");
  } finally {
    button.disabled = false;
    button.textContent = "產生旁白";
  }
}

async function downloadRemoteFile(url, filename) {
  if (!url) return showToast("請先產生旁白");
  const response = await fetch(url);
  if (!response.ok) return showToast("檔案下載失敗");
...
function bindEvents() {
  ...
  $("#recordWebmBtn").addEventListener("click", () => recordVideo("webm"));
  $("#recordMp4Btn").addEventListener("click", () => recordVideo("mp4"));
  ...
  $("#downloadNarrationBtn").addEventListener("click", () =>
    downloadRemoteFile(state.narration.audioUrl, `${safeName(els.projectTitle.value)}-旁白.mp3`));
  $("#downloadSubtitleBtn").addEventListener("click", () =>
    downloadRemoteFile(state.narration.subtitleUrl, `${safeName(els.projectTitle.value)}-字幕.srt`));
  els.narrationAudio.addEventListener("loadedmetadata", () => {
    state.narration.duration = Number.isFinite(els.narrationAudio.duration) ? els.narrationAudio.duration : 0;
    $("#narrationDuration").textContent = formatTime(state.narration.duration);
  });
  $("#previewVolume").addEventListener("input", event => {
    els.narrationAudio.volume = Number(event.target.value) / 100;
  });
